...
def run_ui():
    ...
    def validate_and_run():
        ...
        outpath = output_path_var.get()
        try:
            height = int(height_var.get())
            window_size = int(window_size_var.get())
            correct_perspective = int(perspective_var.get())
    ...
    tk.Label(root, text="Video File:").grid(row=0, column=0, sticky="w", padx=10, pady=5)
    video_path_var = tk.StringVar()
    tk.Entry(root, textvariable=video_path_var, width=40).grid(row=0, column=1, padx=10, pady=5)
    ...
    # Other parameters
    tk.Label(root, text="Height:").grid(row=2, column=0, sticky="w", padx=10, pady=5)
    height_var = tk.StringVar(value="720")
    tk.Entry(root, textvariable=height_var).grid(row=2, column=1, padx=10, pady=5)

    tk.Label(root, text="Window Size:").grid(row=3, column=0, sticky="w", padx=10, pady=5)
    window_size_var = tk.StringVar(value="900")
    tk.Entry(root, textvariable=window_size_var).grid(row=3, column=1, padx=10, pady=5)

    tk.Label(root, text="Correct Perspective (1/0):").grid(row=4, column=0, sticky="w", padx=10, pady=5)
    perspective_var = tk.StringVar(value="0")
    tk.Entry(root, textvariable=perspective_var).grid(row=4, column=1, padx=10, pady=5)

    tk.Label(root, text="Alpha:").grid(row=5, column=0, sticky="w", padx=10, pady=5)
    alpha_var = tk.StringVar(value="0.5")
    tk.Entry(root, textvariable=alpha_var).grid(row=5, column=1, padx=10, pady=5)

    # Run button
    tk.Button(root, text="Run", command=validate_and_run, bg="green", fg="white").grid(row=6, column=1, pady=20)
